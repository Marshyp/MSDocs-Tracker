...
function getToken(): string | null {
  const u = new URL(window.location.href); return u.searchParams.get('token');
}
async function fetchJson<T>(url: string, init: RequestInit = {}): Promise<T> {
  const headers: Record<string, string> = { Accept: 'application/vnd.github+json', ...(init.headers as any) }
  const token = getToken(); if (token) headers.Authorization = `Bearer ${token}`;
  const res = await fetch(url, { ...init, headers }); if (!res.ok) throw new Error(`GitHub ${res.status}: ${url}`);
...
export async function searchMergedPRsMulti(repos: string[], sinceISO: string, query: string, perPage = 100) {
  // Use OR to combine repos safely
  const repoExpr = repos.map(r => `repo:${r}`).join(' OR ');
  const q = `(${repoExpr}) is:pr is:merged merged:>=${sinceISO}${query ? ` ${query}` : ''}`;
  const url = `/api/search?q=${encodeURIComponent(q)}&per_page=${perPage}&since=${sinceISO}`;
  try { return await fetchJson<{ items: PRItem[] }>(url) }
  catch { const direct = `https://api.github.com/search/issues?q=${encodeURIComponent(q)}&sort=updated&order=desc&per_page=${perPage}`; return await fetchJson<{ items: PRItem[] }>(direct) }
}
